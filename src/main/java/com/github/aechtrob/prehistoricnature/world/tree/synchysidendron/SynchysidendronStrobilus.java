...
public class SynchysidendronStrobilus extends PNDecayableDirectional implements IEnvelopable {
    ...
    public SynchysidendronStrobilus(Properties properties) {
        super(properties);
    ...
    public void randomTick(BlockState state, ServerLevel world, BlockPos pos, RandomSource rand) {
        if (!state.getValue(PERSISTENT)) {
            if (world.getBlockState(pos.relative(state.getValue(DirectionalBlock.FACING).getOpposite())).getBlock()
                != BlocksTreeSynchysidendron.SYNCHYSIDENDRON_LEAVES.value()) {
    ...
    public static VoxelShape getShape(BlockState state) {
        return AABBS.get(state.getValue(DirectionalBlock.FACING));
    }
    ...
    public ItemStack envelopeMimicsItemPlacement() {
        return new ItemStack(BlocksTreeSynchysidendron.SYNCHYSIDENDRON_SAPLING.get(), 1);
    }
