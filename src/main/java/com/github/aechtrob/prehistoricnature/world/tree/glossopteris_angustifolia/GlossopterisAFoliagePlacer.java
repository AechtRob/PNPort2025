...
public class GlossopterisAFoliagePlacer extends BlobFoliagePlacer {
    public GlossopterisAFoliagePlacer(IntProvider p_161356_, IntProvider p_161357_, int p_161358_) {
        super(p_161356_, p_161357_, p_161358_);
    }

    public static final Codec<GlossopterisAFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> {
        return blobParts(instance).apply(instance, GlossopterisAFoliagePlacer::new);
    });

    @Override
    protected FoliagePlacerType<?> type() {
        return PNFoliagePlacerType.GLOSSOPTERISA_FOLIAGE_PLACER.get();
    }

    BlockState leafState = BlocksTreeGlossopterisA.GLOSSOPTERISA_LEAVES.get().defaultBlockState();
    BlockState strobilusState = BlocksTreeGlossopterisA.GLOSSOPTERISA_STROBILUS.get().defaultBlockState().setValue(PNDecayableDirectional.PERSISTENT, false);
