...
public class BothrodendronStrobilus extends PNDecayableDirectional implements IEnvelopable {
    ...
    public BothrodendronStrobilus(Properties properties) {
        super(properties);
    ...
    public void randomTick(BlockState state, ServerLevel world, BlockPos pos, RandomSource rand) {
        if (!state.getValue(PERSISTENT)) {
            if (world.getBlockState(pos.relative(state.getValue(DirectionalBlock.FACING).getOpposite())).getBlock()
                != BlocksTreeBothrodendron.BOTHRODENDRON_LEAVES.value()) {
    ...
    public ItemStack envelopeMimicsItemPlacement() {
        return new ItemStack(BlocksTreeBothrodendron.BOTHRODENDRON_SAPLING.get(), 1);
    }
